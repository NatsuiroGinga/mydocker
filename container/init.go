...
func RunContainerInitProcess() error {
	...
	if err := syscall.Exec(path, cmdArray, os.Environ()); err != nil {
		logrus.Errorf("%s", "RunContainerInitProcess exec :"+err.Error())
	}
	return nil
}
...
func setUpMount() {
	...
	err = pivotRoot(pwd)
	if err != nil {
		logrus.Errorf("pivotRoot failed,detail: %v", err)
		return
	}
...
func pivotRoot(root string) error {
	if err := syscall.Mount(root, root, "bind", syscall.MS_BIND|syscall.MS_REC, ""); err != nil {
		return errors.Join(err, errors.New("mount rootfs to itself"))
	}
	...
	if err := syscall.PivotRoot(root, pivotDir); err != nil {
		return errors.Join(err, fmt.Errorf("pivotRoot failed,new_root:%v old_put:%v", root, pivotDir))
	}
	// 修改当前的工作目录到根目录
	if err := syscall.Chdir("/"); err != nil {
		return errors.Join(err, errors.New("chdir to / failed"))
	}
	...
	if err := syscall.Unmount(pivotDir, syscall.MNT_DETACH); err != nil {
		return errors.Join(err, errors.New("unmount pivot_root dir"))
	}
	// 删除临时文件夹
